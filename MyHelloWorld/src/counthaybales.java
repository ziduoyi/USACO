...
public class counthaybales {
	...
	static void init(int n, int[] arr) {
		...
		for (int i = 1; i < 4 * N; i++)
			segs[i][3] = Integer.MAX_VALUE;

		nums = arr;
	}

	static void pushup(int x, int s1, int s2){
		segs[x][0] = segs[s1][0] + lazys[s1]*(segs[s1][2]-segs[s1][1]+1) + segs[s2][0] + lazys[s2]*(segs[s2][2]-segs[s2][1]+1); // push up
		segs[x][3] = Math.min(segs[s1][3] + lazys[s1], segs[s2][3] + lazys[s2]); // push up
	}
	...
	static void buildTreeS(int x, int l, int r) {
		...
		segs[x][2] = r;
		if (l == r){
			segs[x][0] = nums[l];segs[x][3] = nums[l];
		}else {
			int mid = l + (r - l) / 2;
			int s1 = x * 2, s2 = s1 + 1;
			buildTreeS(s1, l, mid);
			buildTreeS(s2, mid + 1, r);
			segs[x][0] = segs[s1][0] + segs[s2][0]; // push up
			segs[x][3] = Math.min(segs[s1][3], segs[s2][3]); // push up
		}
	}
	...
	static long queryS(int x, int a, int b) {
		...
		if (a <= l && r <= b)
			return segs[x][0] + (lazys[x]) * (r - l + 1);// delta * (j-i+1) +
		else {
			int mid = (l + r) / 2;
	...
	static long queryM(int x, int a, int b) {
		...
		if (a <= l && r<=b) {
			...
			a = Math.max(a, l);b = Math.min(r, b);
			if (a <= mid)
				res = Math.min(res, queryM(s1, a, Math.min(b, mid))+lazys[x]); // l,
			if (mid < b)
				res = Math.min(res, queryM(s2, Math.max(a, mid + 1), b)+lazys[x]);
		}
		return res;
	}
}
